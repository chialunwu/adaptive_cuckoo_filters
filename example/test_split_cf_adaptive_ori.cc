...
//			fpp_filters[hash1]++;
			...
			if(!filter[hash1]){
			    filter[hash1] = new CuckooFilter<char[bytes_per_item], 8> (single_cf_size);
	   ...
	   if(t2.size() >=2 ){
		...
		if(record.length() < 256){
			...
			if(adaptive){
				...
				//Check cuckoo filter
				hash1 = hash1 % max_blocks;
				if (filter[hash1] && filter[hash1]->Contain(index, tag, &r_index) == cuckoofilter::Ok && true_negative_flag) {
				    false_queries++;
				    fpp_filters[hash1]++;
				    //cout << record << endl;
				}else{
