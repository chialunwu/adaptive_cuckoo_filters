...
//	CuckooFilter<size_t, 8> filter((size_t)(mem_budget/(4*bits_per_tag/8)), true);
	CuckooFilter<size_t, bits_per_tag> filter(total_items, false);
...
//	cout << record << '\n';
	...
	if(type == "BoscI:"){
	   if(t2.size() >= 3){
		...
		if(mapping_table.find(record) == mapping_table.end() && num_inserted < total_items){
			size_t raw_index;
			uint32_t index, tag;
